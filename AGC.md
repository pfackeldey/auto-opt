...
`"$HOME/.pixi/bin/pixi" run python agc_processor.py`

**Optimization objective:**

Events/s, runtime in seconds, and peakRSS usage in bytes.
...
All that are available in the existing env, do not add new dependencies.

**Additional hints / custom instructions (higher priority than the default optimization rules below):**

Treat these as user-specific direction for this run. Follow them when they
conflict with a default preference, while still preserving correctness and
measuring the result.

In case there's an issue or a bug with one patch/change that you're making, try to fix it before going to the next optimization step.

- Do NOT switch the `mode="eager"` in the NanoEventsFactory. Keep it ALWAYS to `"virtual"`.
- Change entry start and stop to process as many events as possible in the given ressource constraints
- You can use `buffer_cache=mapping` and inside of the processor do `mapping.pop(<key>)` to evict buffers as soon as they're not needed anymore for subsequent computations. The key names are a bit technical, but you can match the branch/column names with a regex. That should reduce peak memory usage.
- Write custom C extensions for any bottlenecks (including replacing dependencies such as awkward or uproot), bind with ctypes. Do at LEAST 3 steps out of the maximum step attempts that try custom C code!
- Try using Numba compiled loops, they can be used with numpy arrays or even directly with Awkward Arrays
- Always report per step: Events/s, runtime in seconds, and peakRSS usage in bytes.
...
- Do not perform unrelated cleanups, formatting-only edits, or speculative architecture changes unless smaller changes are exhausted.
...
Run until the max optimization attempts is reached. Do not stop before(!), try every idea you can think of, this loop runs automatically and the user can't prompt/reply/give access to you.

This is a non-interactive run. Do not ask the user questions, wait for a reply,
or pause between steps for approval. When a choice is needed, use the safest
reasonable default consistent with this prompt and record it in the step notes.
If a required action is blocked (for example, unavailable credentials,
permissions, or toolchain), record the concrete failure, reject that attempt,
and continue with the next feasible optimization until the attempt limit is
reached.
...
- If the candidate is a clean commit, use `git -c user.name=auto-opt -c user.email=auto-opt@local revert <commit>`.
...
Finally, create simple scientific plots with maplotlib in `.auto-opt/plots/` that show performance improvements per step including a short label per data point that explaines what was improved. Also add degrations to the figure, but mark them in a different style so it's clear they were rejected. Make sure everything is well formatted and readable in the plots.
The last thing you do is one final commit, such that a new optimization loop will start fresh when going back to main or a new optimization branch.
